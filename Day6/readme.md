...
* When individual scores were accidentally left inside the feature matrix during initial testing, the model suffered from absolute **Data Leakage**, showing a deceptive performance of `MAE: 0.00` and `R²: 1.0000`.
* After correctly dropping individual test scores and isolating the target variable, the model yielded realistic, organic performance metrics:
    * **Mean Absolute Error (MAE)**: `0.19`
    * **Mean Squared Error (MSE)**: `0.16`
    * **R² Score**: `0.9980`
* This high R² score is organic as the remaining features retain an incredibly strong linear relationship with the target array, allowing the model to predict student outcomes within an average margin of just 0.19 marks.
